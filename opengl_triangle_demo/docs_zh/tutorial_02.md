...
### 扩充冰箱里的大象步骤-1（接口 vs 实现、窗口管理）

想要绘制三角形，我们需要使用 `OpenGL` 提供的接口；
但问题来了：OpenGL 只是个标准，它只告诉你**函数长什么样**，而真正的实现藏在 GPU 驱动里。

这就像你有一张“电话簿”，上面写着：
- 有个叫 drawTriangle 的人，电话号码是 XXX-XXXX。

可是不同的系统（Windows/macOS/Linux），电话号码都不一样。
如果你不知道去哪里查，就根本打不通。

所以我们需要一本“自动查电话簿”的字典工具——这就是 **GLAD**。
它在运行时帮你找到 GPU 驱动里真正的函数地址。
GLAD 本身提供了 `glad.c` / `glad.h` / `khrplatform.h`，我们只要把这些文件丢进工程，就能跨平台使用 OpenGL。

> 💡 面试小问答：  
> **Q:** 为什么需要 GLAD，而不是直接用 OpenGL 的头文件？  
> **A:** 因为 OpenGL 的标准头文件只声明函数接口，并不保证你能拿到实现。GLAD 通过加载函数指针，确保不同系统都能调用到真正的 GPU 驱动实现。

仅仅有 `OpenGL` 接口还不够，还需要一个能创建窗口、处理键盘鼠标事件的库——`GLFW`。
它就像冰箱的门铰链，没有它根本没法“开门放象”。

**到现在为止，文件准备步骤扩充为：**
导入字典（GLAD） → 打开窗口（GLFW） → 绘制三角形 → 关闭窗口
...
写出来的工程文件，必须能在 Windows、macOS、Linux 里都顺利打开。
这时我们需要一份“多语言翻译手册”—— `CMakeLists.txt`。
它能告诉不同的编译器：源文件放哪、库在哪、头文件在哪。这样才不会出现“同一个冰箱，结果 Windows 说打不开门，macOS 说找不到门把手”。
